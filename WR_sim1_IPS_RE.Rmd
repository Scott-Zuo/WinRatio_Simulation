---
title: "Win Ratio IPS Relative Efficiency"
author: "Scott Zuo"
output: 
    html_document:
      toc: TRUE
...
## Relative Efficiency for the original and IPS Weighted Estimator
...
WR_sim_val0_IPS <- numeric(re_count)
WR_sim_valInf_IPS <- numeric(re_count)
WR_sim_valNA_IPS <- numeric(re_count)
...
df_trt_1col_IPS <- data.frame(outcomes_trt)
df_ctrl_1col_IPS <- data.frame(outcomes_ctrl)

colnames(df_trt_1col_IPS) <- "outcomes_comb"
colnames(df_ctrl_1col_IPS) <- "outcomes_comb"

df_comb_1col <- rbind(df_trt_1col_IPS, df_ctrl_1col_IPS)
...
trtwin_1col_IPS <- matrix(NA, nrow = nrow(df_trt_1col_IPS), ncol = nrow(df_ctrl_1col_IPS))
ctrlwin_1col_IPS <- matrix(NA, nrow = nrow(df_trt_1col_IPS), ncol = nrow(df_ctrl_1col_IPS))

for (i in 1:n_count){
  for(j in (n_count+1):(2*n_count)){

      if (df_comb_1col[i,1] > df_comb_1col[j,1]){
        trtwin_1col_IPS[i,j-n_count] = (1/(pi_func[i]*(1-pi_func[j])))/2
        ctrlwin_1col_IPS[i,j-n_count] = 0
      }
      else if (df_comb_1col[i,1] < df_comb_1col[j,1]){
        trtwin_1col_IPS[i,j-n_count] = 0
        ctrlwin_1col_IPS[i,j-n_count] = (1/(pi_func[i]*(1-pi_func[j])))/2
      }
...
trt_winpr_1col_IPS <- sum(as.vector(trtwin_1col_IPS), na.rm = T)/(choose(2*n_count,2))
ctrl_winpr_1col_IPS <- sum(as.vector(ctrlwin_1col_IPS), na.rm = T)/(choose(2*n_count,2))

WR_sim_1col_IPS[sim_count] <- trt_winpr_1col_IPS/ctrl_winpr_1col_IPS
...
bias_IPS[count_temp] <- mean(WR_sim_noninf_1col_IPS) - true_wr_1col
mse_IPS[count_temp] <- bias_IPS[count_temp]^2 + empirical_var_IPS[count_temp]


WR_sim_val0_IPS[count_temp] <- length(WR_sim_1col_IPS[WR_sim_1col_IPS = 0])
WR_sim_valInf_IPS[count_temp] <- length(WR_sim_1col_IPS[WR_sim_1col_IPS = Inf])
WR_sim_valNA_IPS[count_temp] <- length(WR_sim_1col_IPS[is.na(WR_sim_1col_IPS)])

}

relative_eff <- empirical_var_ori/empirical_var_IPS

save(re_count, relative_eff,
     empirical_var_ori, bias_ori, mse_ori,
     empirical_var_IPS, bias_IPS, mse_IPS,
     WR_sim_val0_ori, WR_sim_valInf_ori, WR_sim_valNA_ori,
     WR_sim_val0_IPS, WR_sim_valInf_IPS, WR_sim_valNA_IPS, file = "IPS_RE_Output.RData")
```
...
sim_df <- rbind(var_ori_df, var_IPS_df)
sim_df$mse <- c(mse_ori, mse_IPS)
sim_df$bias <- c(bias_ori, bias_IPS)
...
re_plot <- ggplot(re_df, aes(x=samplesize, y = relative_eff)) + 
  ...
  ggtitle("Relative Efficiency from Simulation, IPS vs Original") +
