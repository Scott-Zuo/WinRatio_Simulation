...
true_logodds1_ctrl <- 1 + 0.5 * 1
true_logodds2_ctrl <- 0.05 + 0.5 * 1
...
# 0.1824255 0.1834389 0.6341356


#trt: 0.07585818 0.09922809 0.82491373
#ctrl: 0.1824255 0.1834389 0.6341356

true_trt_wp_1col <- 0.82491373*(0.1824255+0.1834389) + 0.09922809*0.1824255
true_ctrl_wp_1col <- 0.6341356*(0.09922809+0.07585818) + 0.1834389*0.07585818
...
WR_sim_noninf_1col <- WR_sim_1col[WR_sim_1col!=Inf & !is.na(WR_sim_1col)]
...
df_comb_1col$treatment <- c(rep(1,10), rep(0,10))
#df_comb_1col$trt_eff <- c(rep(0.5,10), rep(0,10))
...
for (i in 1:10){
  for(j in 11:20){
      ...
      if (df_comb_1col[i,1] > df_comb_1col[j,1]){
        trtwin_1col[i,j-10] = (1/(pi_func[i]*(1-pi_func[j])))/2
        ctrlwin_1col[i,j-10] = 0
      }
      else if (df_comb_1col[i,1] < df_comb_1col[j,1]){
        trtwin_1col[i,j-10] = 0
        ctrlwin_1col[i,j-10] = (1/(pi_func[i]*(1-pi_func[j])))/2
      }
...
trt_winpr_1col <- sum(as.vector(trtwin_1col), na.rm = T)/(choose(20,2))
ctrl_winpr_1col <- sum(as.vector(ctrlwin_1col), na.rm = T)/(choose(20,2))
...
WR_sim_noninf_1col_IPS <- WR_sim_1col_IPS[WR_sim_1col_IPS!=Inf & !is.na(WR_sim_1col_IPS)]
...
line.data_1col <- data.frame(xintercept = c(mean(WR_sim_noninf_1col), median(WR_sim_noninf_1col), true_wr_1col), 
                        Lines = c("Mean", "Median", "True_WinRatio"),
...
WR_hist1_1col <- ggplot(WR_sim_df_1col, aes(x=WR_sim_noninf_1col)) + 
  geom_histogram(color="black", fill="white", binwidth = 1, aes(y=after_stat(density))) +
  xlim(0, 25) +
  geom_density(alpha=0.2, fill="salmon") +
  geom_vline(aes(xintercept = xintercept, color = Lines), line.data_1col, linewidth = 0.5) +
...
WR_hist1_1col_IPS <- ggplot(WR_sim_df_1col_IPS, aes(x=WR_sim_noninf_1col_IPS)) + 
  ...
  xlim(0, 25) +
  geom_density(alpha=0.2, fill="salmon") +
  geom_vline(aes(xintercept = xintercept, color = Lines), line.data_1col_IPS, linewidth = 0.5) +
